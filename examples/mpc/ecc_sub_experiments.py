...
def run(gui=False, plot=True, n_episodes=1, n_steps=None, save_data=True):
    ...
    for i in range(3):
        ...
        if i == 1:
            config.task_config.init_state.init_x += 0.15
            config.task_config.init_state.init_y += 0.15
            config.task_config.init_state.init_z += 0.15
        elif i == 2:
            config.task_config.init_state.init_x -= 0.15
            config.task_config.init_state.init_y -= 0.15
            config.task_config.init_state.init_z -= 0.15
        ...
        if config.algo == 'mpc_acados':
            filename = f"p{i}_{datetime.now().strftime('%d_%m')}_trajscal_{config.task_config.task_info.trajectory_scale}_acados_N{config.algo_config.horizon}_RTI_{config.algo_config.use_RTI}_init_{config.algo_config.initial_guess_t0.initialization_type}_with_{config.algo_config.initialization}"
        elif config.algo == 'mpc_acados_m':
            if config.algo_config.second_phase.method == "rl":
                filename = f"p{i}_{datetime.now().strftime('%d_%m')}_trajscal_{config.task_config.task_info.trajectory_scale}_rl_ppo_timed_l4casadi"
